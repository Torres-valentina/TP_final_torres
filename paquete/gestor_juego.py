...
from paquete.logica_juego import inicializar_estado_juego_cartas 
...
def _inicializar_estado_juego_completo(pantalla, fuente, reverso, imagenes_cartas,
    ...
    estado = {
        ...
        "musica_activada": musica_activada_inicial, 
        "icono_actual": icono_actual_inicial,       
        "movimientos": 0,
        "corriendo": True # bandera para controlar el bucle principal del juego
    }
    return estado

# --- Funciones Auxiliares para el manejo de clics ---
...
def _manejar_clic_boton_control(pos, estado_juego):
    ...
    elif BOTON_VOLVER_RECT.collidepoint(pos):
        estado_juego["corriendo"] = False # Esto hará que el bucle del juego termine
        bandera = True

    elif BOTON_SIGUIENTE_RECT.collidepoint(pos):
...
def _manejar_clic_mazo(pos, estado_juego):
    ...
    elif MAZO_RECT.collidepoint(pos):
        if mazo:
...
def _manejar_movimiento_carta_seleccionada(pos, estado_juego):
    ...
    elif _intentar_mover_mazo_a_pila(pos, estado_juego):
        bandera = True

    elif _intentar_mover_pila_a_pila(pos, estado_juego):
        bandera = True
    ...
    for i, (ocultas, visibles) in enumerate(pilas):
        ...
        for j, carta in enumerate(visibles):
            ...
            if rect_carta.collidepoint(pos):
                ...
                return #detiene la funcion cuando encuentra la carta clickeada

def _manejar_clic_mouse(pos, estado_juego):
    """
...
def _manejar_eventos_juego(estado_juego):
    ...
    for evento in pygame.event.get():
        if evento.type == pygame.QUIT:
            ...
            _manejar_clic_mouse(evento.pos, estado_juego)
